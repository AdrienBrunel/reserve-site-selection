  ...
  ## Computation ---------------------------------------------------------------
    ...
    for i in 1:Ny
      for j in 1:Nx
        ...
        FC_bnb = 99
        FC_min = min(fishing_intensity...)
        FC_max = max(fishing_intensity...)
        FC_bwd = (FC_max-FC_min)/FC_bnb
        for k in 1:FC_bnb
          if fishing_intensity[i,j] == FC_min
            cost_fshscale[cpt] = 1
          else
            if (fishing_intensity[i,j] > FC_min + (k-1)*FC_bwd) & (fishing_intensity[i,j] <= FC_min + k*FC_bwd)
              cost_fshscale[cpt] = k+1
            end
          end
        end
      end
